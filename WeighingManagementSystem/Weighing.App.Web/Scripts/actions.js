...
$(document).ready(function () {
    ...
    $(".page-navigation-toggle").on("click", function () {
        //OSCAR - TOOGLE MENU OPEN CLOSE
        //alert('CURRENT: ' + document.cookie);

        if (document.cookie == 'NavBar=Closed' || document.cookie == 'NavBar=Closed; NavBar=Closed' || document.cookie == 'NavBar=Closed; NavBar=Opened') {
            //alert('MAKE IT OPEN');
            deleteCookie('NavBar');
            document.cookie = 'NavBar=Opened';
        } else if (document.cookie == "NavBar=Opened" || document.cookie == "NavBar=Opened; NavBar=Opened") {
            //alert('MAKE IT CLOSE');
            deleteCookie('NavBar');
            document.cookie = 'NavBar=Closed';
        }
        else {
            //alert('4');
            deleteCookie('NavBar');
